...
##Loading dataset and read csv:
```{r}
train_dataset<-read.csv("C:/Users/sudhakar/Downloads/train_v3.csv")
```


##Creating a default column, if loss is 0 then defalut 0
##if loss is more than 0 then default is 1
```{r}
train_dataset$default <- factor(ifelse(train_dataset$loss > 0, 1, 0))

train_dataset$loss <- (train_dataset$loss / 100)
```

##Checking the missing values in the data set:
```{r}
row_missing <- rowMeans(is.na(train_dataset))
...
ggplot(train_dataset, aes(x=factor(default))) +
  geom_bar(stat="count", width=0.4, fill="lightblue") +
...
```{r}
zero_var<- nearZeroVar(train_dataset[ ,-c(763,764)])

data_cleaned <- train_dataset[, -zero_var_indices]

bank_preprocess <- preProcess(data_cleaned[ ,-c(739,740)], method = c("corr", "medianImpute"))

new_train_dataset <- predict(bank_preprocess, data_cleaned)
```

##Removing zero-variance,highly corr variables and imputed missing values : we have new data set "new_train_dataset" with 248 attributes.
...
#a).Lasso Model: we now run the new_train_dataset with 248 attributes for variable selection:
```{r}
set.seed(123)

y <- as.vector(as.factor(new_train_dataset$default))

x <- data.matrix(new_train_dataset[,-c(247,248)])
...
#Selecting attributes from original data set "new_train_dataset" using coefficients returned from lasso model i.e., "new_bank_coef"
bank_lasso<-select(new_train_dataset, new_bank_coef)
...
```{r}
pca_test <- read.csv("C:/Users/sudhakar/Downloads/test__no_lossv3.csv")
```
...
```{r}
new_train <- read.csv("C:/Users/sudhakar/Downloads/train_v3.csv")
```
